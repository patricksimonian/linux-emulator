...
export const writeFile: CommandFn = async (
    raw,
    command,
    args,
    context,
    ...
    if (file) {
        ...
        if (file.locked && file.locked.userId !== accountName) {
            return { stdout: '', stderr: `writeFile: cannot write to ${fullPath}. It is locked.`, code: 1 };
        }

        const currentSize = file.content?.length || 0;
        const newSize = content.length;
        const sizeDiff = newSize - currentSize;

        if (sizeDiff > 0) {
            try {
                checkQuota(context, sizeDiff, 0);
            } catch (e: unknown) {
        ...
        updateUsage(context, sizeDiff, 0);
        return { stdout: '', code: 0 };
    } else {
        try {
            checkQuota(context, content.length, 1);
        } catch (e: unknown) {
            return { stderr: `${command}: ${(e as Error).message}\n`, stdout: '', code: 1 };
        }
